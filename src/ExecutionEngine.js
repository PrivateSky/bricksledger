function getCommandMethodExecution(command, contracts, createFSKeyValueStorage, rootFolder) {
    const { domain, contractName, methodName, params } = command;
    const contract = contracts[contractName];

    const keyValueStorage = createFSKeyValueStorage(domain, rootFolder, contract.name);
...
async function loadContract(executionEngine, rawDossier, contractConfig) {
    let contract;
...
function setContractMixin(executionEngine, contractName, contract) {
...
class ExecutionEngine {
    ...
    async loadContracts() {
        ...
        for (let i = 0; i < contractConfigs.length; i++) {
            ...
            const contract = await loadContract(this, rawDossier, contractConfig);
    ...
    async validateCommand(command) {
        const { domain, contractName, methodName, params, type, timestamp, signerDID: signerDIDIdentifier } = command;
        if (this.domain !== domain) {
            throw `Invalid domain specified`;
        }

        if (!contractName || typeof contractName !== "string" || !this.contracts[contractName]) {
        ...
        const contractMethodsInfo = this.describeMethodsForContract(contractName);
        ...
        if (type === "nonced") {
            ...
            if (!timestamp || !signerDIDIdentifier || typeof signerDIDIdentifier !== "string") {
            ...
            const isCommandRegistered = await this.commandHistoryStorage.isCommandHashRegistered(commandHash);
    ...
    executeMethodOptimistcally(command) {
        const executionResult = getCommandMethodExecution(command, this.contracts, this.createFSKeyValueStorage, this.rootFolder);
        return executionResult;
    }
    ...
    async executePBlock(pBlock) {
        try {
            ...
            for (let i = 0; i < commands.length; i++) {
                ...
                const contractMethodExecutionPromise = getCommandMethodExecution(
                    command,
                    this.contracts,
                    this.createFSKeyValueStorage,
                    this.rootFolder
                );
                await contractMethodExecutionPromise;
            }
        } catch (error) {
            throw error;
        }
