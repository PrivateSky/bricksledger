...
function FSKeyValueStorage(basePath, type) {
    ...
    this.updateValidated = async function (key, commandHash, newValueObject, callback) {
        let fp = getKeyPath(key);
        let str = await require("fs").readFile(fp);
        let value = new StorageValue(str);
        value.updateValidated(commandHash, newValueObject);
        fs.writeFile(fp, value.asString(), callback);
    };

    this.addPending = async function (key, commandHash, newValueObject, callback) {
        let fp = getKeyPath(key);
        let str = await require("fs").readFile(fp);
        let value = new StorageValue(str);
    ...
    this.getValue = async function (key, latest, callback) {
        let fp = getKeyPath(key);
        let str = await require("fs").readFile(fp);
        let value = new StorageValue(str);
