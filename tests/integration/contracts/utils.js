...
async function launchApiHubTestNodeWithTestDomainAsync(config) {
    if (!config) {
        config = {};
    }
    ...
    const validatorURL = process.env[constants.BDNS_ROOT_HOSTS];

    if (!config.validators) {
        config.validators = [{ DID: validatorDID.getIdentifier(), URL: validatorURL }];
    }
...
module.exports = {
    launchApiHubTestNodeWithTestDomainAsync,
};
